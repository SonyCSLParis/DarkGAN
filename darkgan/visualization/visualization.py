...
vis = visdom.Visdom(raise_exceptions=False, port=8097)
...
class AttClassifVisualizer(TensorVisualizer):
    ...
    def publish(self, true, fake, name, title):
        ...
        for i, att in enumerate(self.attributes):
            if self.att_val_dict[att]['type'] not in [str(int), str(str), ["<class 'list'>", "<class 'int'>"]]:
                continue
            win = att + name
            self.update_tokens(win)
...
class LossVisualizer(TensorVisualizer):
    ...
    def publish_cpc_codes(self, codes, win='cpc'):
        
        # if not hasattr(self, 'cpc'):
        #     self.cpc = {}
        # if win not in self.cpc:
        #     self.cpc[win] = {'x': [], 'y': []}


        for i in range(len(codes)):
            opts = {'title': win  + str(i),
                'xlabel': 'frames', 'ylabel': 'cluster'}
            self.update_tokens(win + str(i))
            self.window_tokens[win + str(i)] = vis.line(X=list(range(len(codes[i]))), Y=codes[i], opts=opts,
                                       win=self.window_tokens[win + str(i)], env=self.env + '_cpc')
        # if output_dir:
        #     self.save(iter_n=self.score[name]['x'],
        #             loss_val=self.score[name]['y'],
        #             title=name,
