...
def walk_files_abs_path(dir_path, format='.wav'):
    path = Path(dir_path)
    return list(path.glob(f'**/*{format}'))
...
def saveAudioBatch(data, path, basename, sr=16000, overwrite=False, _format='wav'):
...
class ResizeWrapper():
    ...
    def __call__(self, image):
        ...
        if len(image.size()) == 3:
            ...
            out = interpolate(image, size=self.size)
        return out
...
def get_trainer(name):
    ...
    match = {"PGAN": ("progressive_gan_trainer", "ProgressiveGANTrainer"),
             ...
             "CPCGAN":("cpc_gan_trainer", "CPCGANTrainer"),
