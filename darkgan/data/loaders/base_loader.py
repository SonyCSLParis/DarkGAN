...
class DataLoader(ABC, data.Dataset):
    def __init__(self,
        ...
        self.output_path = mkdir_in_path(os.path.expanduser(output_path), self.dbname)
    ...
    def dump(self):
        ...
        tr_path = mkdir_in_path(self.output_path, 'train')
        val_path = mkdir_in_path(self.output_path, 'val')
        self.data = list(self.data)
        self.metadata = list(self.metadata)

        # if no annotations create list of Nones
        if len(self.metadata) == 0:
            self.metadata = [[-1]] * len(self.data)
        if len(self.val_labels) == 0:
            self.val_labels = [[-1]] * len(self.val_data)

        for i, (d, m) in enumerate(zip(self.data, self.metadata)):
            file = f'{tr_path}/{i}.pt'
            with open(file, 'wb') as f:
                torch.save((d, m), f)
                self.data[i] = file
        for i, (d, m) in enumerate(zip(self.val_data, self.val_labels)):
            file = f'{val_path}/{i}.pt'
            with open(file, 'wb') as f:
                torch.save((d, m), f)
                self.val_data[i] = file

        del self.metadata, self.val_labels
    ...
    def __getitem__(self, index):
        data, labels = torch.load(self.data[index])
        labels = torch.Tensor(labels)
        data = torch.from_numpy(data)
        if self.lazy_loading:
            data = self.preprocessing(data)
        if self.getitem_processing:
            data = self.getitem_processing(data)
        return data, labels
        ...
        for j, b in enumerate(batch):
            ...
            for i, att_dict in enumerate(self.header['attributes'].values()):
                if att_dict['type'] == str(float):
                    ...
                    shift += len(att_dict['values'])           
                elif att_dict['type'] == (str(list), str(int)):
                    
                    labels += [b[shift:shift+att_dict['seq_len']]]
                    shift += att_dict['seq_len']
                else:
    ...
    def train_val_split(self, tr_val_split=0.9):
        ...
        if self.lazy_loading:
            self.val_data = list(map(self.preprocessing, self.val_data))
    ...
    def get_validation_set(self, batch_size=None, process=False):
        ...
        val_batch = choices(self.val_data, k=batch_size)
        
        val_data = []
        val_labels = []
        for v in val_batch:
            data, label = torch.load(v)
            val_data.append(torch.Tensor(data).unsqueeze(0))
        ...
        if process:
            val_data = self.getitem_processing(val_data)
                # torch.stack([self.getitem_processing(v.unsqueeze()) for v in val_data])
        else:
            pass
            # val_batch = \
