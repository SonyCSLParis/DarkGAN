...
csl_keys = ['instrument', 'tags', 'audio-commons', 'soft_instrument']
csl_keys.sort()
...
audio_commons_keys = [
            # 'duration', 'loudness', 'dynamic_range', \
        ...
        if any(f.endswith('.wav') for f in files):
            ...
            for file in files:
                ...
                for att in csl_keys:
                    ...
                    elif att == 'soft_instrument':
                        if att not in ac_atts: continue
                        out_item['attributes'][att] = ac_atts[att]
                        attributes[att]['values'] = csl_instruments
                i+=1
...
def extract(path: str, criteria: dict={}, download: bool=False):
    ...
    for att in out_attributes:
        ...
        if att in criteria.get('filter', {}).keys():
            ...
            attribute_dict[att]['values'] = standard_desc['attributes'][att]['values'].copy()
        else:
    ...
    for file in pbar:
        ...
        # skip files that do not have all requested attributes
        if not all([x in item_atts for x in attribute_dict.keys()]): continue
