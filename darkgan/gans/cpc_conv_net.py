...
from .custom_layers import EqualizedConv1d, EqualizedConv2d, EqualizedLinear,\
    NormalizationLayer, Upscale2d, GANsynthInitFormatLayer, CPCGANInitFormatLayer
...
class GNet(nn.Module):
    ...
    def initFormatLayer(self, dimLatentVector):
        ...
        # elif self.formatLayerType == 'cpc-gan':
        self.kernelSize = (3, 3)
        self.padding = (1, 1)
        self.formatLayer = CPCGANInitFormatLayer(
    ...
    def forward(self, x, test_all_scales=False):
        ...
        for scale, layerGroup in enumerate(self.scaleLayers, 0):
            ...
            x = self.upscale(x, (self.scaleSizes[scale + 1][0], x.size(-1)))
...
class DNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.scale = 0
        self.n_scales = len(self.inputSizes)
        ...
        self.size_decision_layer = sizeDecisionLayer
    ...
    def initScale0Layer(self):
        self.scale += 1
        dilation = 2**(self.n_scales - self.scale)
        dilation=1
        ...
        self.groupScaleZero.append(EqualizedConv2d(self.depthScale0, self.depthScale0,
                                                   (self.sizeScale0[0], 1), padding=(0, 0),
                                                   equalized=self.equalizedlR,
                                                   initBiasToZero=self.initBiasToZero))

        # self.groupScaleZero.append(EqualizedConv2d(self.depthScale0, 1, # here we have to multiply times the initial size (8 for generating 4096 in 9 scales)
        #                                            (64, 1), padding=(1, 0),
        #                                            equalized=self.equalizedlR,
        #                                            initBiasToZero=self.initBiasToZero))
    ...
    def initDecisionLayer(self, sizeDecisionLayer):
        ...
        #                                      equalized=self.equalizedlR,
        #                                      initBiasToZero=self.initBiasToZero)
        self.decisionLayer = EqualizedConv2d(self.depthScale0,
                                                  sizeDecisionLayer,
                                                  1,
                                                  equalized=self.equalizedlR,
                                                  initBiasToZero=self.initBiasToZero)
    ...
    def forward(self, x, getFeature = False):
        ...
        if self.alpha > 0 and len(self.fromRGBLayers) > 1:
            # hack to avoid downsampling in time dim
            self.inputSizes[len(self.fromRGBLayers) - 2] = (self.inputSizes[len(self.fromRGBLayers) - 2][0], x.size(-1))
            ##### 
        ...
        for i, groupLayer in enumerate(reversed(self.scaleLayers)):
            ...
            self.inputSizes[shift] = (self.inputSizes[shift][0], x.size(-1))
