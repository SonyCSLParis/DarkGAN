...
class GNet(nn.Module):
    ...
    def addScale(self, depthNewScale):
        ...
        if len(self.toRGBLayers) > 2:
            for l in range(len(self.toRGBLayers) - 2):
                # del self.toRGBLayers[l]
                for p in self.toRGBLayers[l].parameters():
    ...
    def forward(self, x, test_all_scales=False):
        ...
        # elif self.alpha == 0 and len(self.toRGBLayers) > 1:
        #     for p in self.toRGBLayers[-2].parameters():
        #         p.requires_grad = False
...
class DNet(nn.Module):
    ...
    def addScale(self, depthNewScale):
        ...
        if len(self.fromRGBLayers) >= 2:
            for l in range(len(self.fromRGBLayers) - 1):
                # del self.fromRGBLayers[l]
                for p in self.fromRGBLayers[l].parameters():
                    p.requires_grad=False
    ...
    def forward(self, x, getFeature = False):
        # Alpha blending
        if self.alpha > 0 and self.n_scales > 1:
            y = self.downScale(x, size=self.inputSizes[self.n_scales - 2])
            y = self.leakyRelu(self.fromRGBLayers[-2](y))
        elif self.n_scales > 1 and self.alpha == 0:
            for p in self.fromRGBLayers[-2].parameters():
                p.requires_grad = False
        # From RGB layer
