...
class ACGANCriterion:
    ...
    def __init__(self,
        ...
        for i, att in enumerate(attribKeysOrder):
            ...
            if attribKeysOrder[att]['loss'] == 'mse':
                ...
                # if attribKeysOrder[att]['type'] == (str(list), str(int)):
                #     self.attribSize.append(attribKeysOrder[att]['seq_len'])
            ...
            elif attribKeysOrder[att]['loss'] == 'soft_xentropy':
                self.attribSize.append(len(attribKeysOrder[att]["values"]))
                self.labelsOrder[att] = {index: label for label, index in
                                     enumerate(attribKeysOrder[att]["values"])}
                self.att_loss.append('soft_xentropy')
            else:
    ...
    def buildLatentCriterion(self, targetCat, skipAtts=False):
        ...
        for i in range(self.nAttrib):
            if skipAtts and self.keyOrder[i] in self.skipAttDfake: 
                if self.att_loss[i] in ['xentropy']:
                    shift2 += 1
            ...
            if self.att_loss[i] == 'xentropy':
                ...
                shift2 += 1
            elif self.att_loss[i] in ['mse', 'bce', 'soft_xentropy']:
    ...
    def getPredictionLabels(self, outputD):
        ########## cpc hack #############
        outputD = outputD.reshape((-1, outputD.size(1)))
        #######################
        ...
        for i in range(self.nAttrib):
            ...
            if self.att_loss[i] in ['xentropy', 'soft_xentropy']:
    ...
    def artificial_soft_xent(self, pred, target, lprob=0.3, hprob=(0.7, 1.2)):
        n_cls = pred.size(1)
    ...
    def soft_xentropy(self, input, target):
        ...
        for i in range(self.nAttrib):
            ...
            if self.keyOrder[i] not in self.skipAttDfake or not skipAtts:
                ...
                if self.att_loss[i] == 'mse':
                    ...
                    shiftTarget += C
                elif self.att_loss[i] == 'soft_xentropy':
                    locTarget = target[:, shiftTarget:shiftTarget + C]
                    locTarget = locTarget.reshape(locInput.size())
                    locLoss = self.soft_xentropy(locInput, locTarget)
                    shiftTarget += C
                
                elif self.att_loss[i] == 'bce':
                    locTarget = target[:, shiftTarget:shiftTarget + C]
                    locInput = torch.sigmoid(locInput/self.temperature)
                ...
                if self.att_loss[i] in ['mse', 'bce', 'soft_xentropy']:
                    shiftTarget += C
                else:
                    shiftTarget += 1
            shiftInput += C
