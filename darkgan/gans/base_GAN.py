...
class BaseGAN():
    ...
    def __init__(self,
        ...
        ####################################################
        if lossMode not in base_loss_criterions.__LOSSES__:
            raise ValueError(
                f"lossMode should be one of the following : {base_loss_criterions.__LOSSES__}")
        if optimizer not in optimizers.__OPTIMIZERS__:
            raise ValueError(
                f"optimizer should be one of the following : {optimizers.__OPTIMIZERS__}")
        ...
        self.optimizer = getattr(
            optimizers, optimizer)(self.device)
    ...
    def updateSolversDevice(self, buildAvG=True):
        ...
        self.optimizer.init_optimizer(G=self.netG, D=self.netD, lr=self.config.learning_rate)
