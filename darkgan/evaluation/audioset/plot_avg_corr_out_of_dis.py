...
def main(out_dir):
	...
	corr_t1 = table_t1.groupby('offset').agg('mean')
	corr_t1['temperature'] = 1
	
	corr_t15 = table_t15.groupby('offset').agg('mean')
	corr_t15['temperature'] = 1.5
	
	corr_t2 = table_t2.groupby('offset').agg('mean')
	corr_t2['temperature'] = 2
	
	corr_t3 = table_t3.groupby('offset').agg('mean')
	corr_t3['temperature'] = 3
	
	corr_t5 = table_t5.groupby('offset').agg('mean')
	...
	# 	axis=0)
	plt.figure()
	ax = sns.lineplot(data=corr_all, x="offset", y="p-t/std", hue="temperature")
	...
	ax.get_figure().savefig(f'{out_dir}/temp_corr_comparison_{datetime.now().strftime("%y_%m_%d_%H:%M:%S")}.pdf')
	plt.close()
